...
export const ShoeViewer: React.FC<ShoeViewerProps> = ({ className = '' }) => {
  ...
  const [zoom, setZoom] = useState(2.5); // Start at 250% zoom
  ...
  const handleReset = () => {
    if (controlsRef.current) {
      ...
      setZoom(2.5); // Reset to 250% zoom instead of 100%
      setAutoRotate(true);
    }
  };
  ...
  return (
    <div className={`relative w-full h-full bg-gradient-viewer rounded-lg overflow-hidden ${className}`}>
      <ErrorBoundary onError={handleModelError}>
        <Canvas
          camera={{ 
            position: [0, 0, 2], // Closer camera position
            fov: 45,
            near: 0.1,
            far: 1000
          }}
        ...
        >
          ...
          <OrbitControls
            ...
            maxDistance={6} // Reduce max distance
