...
interface ColorCustomizerProps {
  ...
  onColorwaySelect?: (colorway: Colorway) => void;
}
...
export const ColorCustomizer: React.FC<ColorCustomizerProps> = ({
  ...
  // Height callback for AIChat positioning
  onHeightChange = () => { },
  // Selected colorway with defaults
  ...
  const handleColorwaySelect = (colorway: Colorway) => {
    // Apply the colorway to all the color states
    onTopColorChange(colorway.upper.color);
    onBottomColorChange(colorway.sole.color);
    onLaceColorChange(colorway.laces.color);
    
    // Apply splatter settings
    onUpperSplatterToggle(colorway.upper.hasSplatter);
    onSoleSplatterToggle(colorway.sole.hasSplatter);
    
    if (colorway.upper.hasSplatter && colorway.upper.splatterColor) {
      onUpperSplatterColorChange(colorway.upper.splatterColor);
    }
    if (colorway.sole.hasSplatter && colorway.sole.splatterColor) {
      onSoleSplatterColorChange(colorway.sole.splatterColor);
    }
    
    // Clear any existing textures when applying colorway
    onUpperTextureChange(null);
    onSoleTextureChange(null);
    
    // Disable gradients to ensure clean colorway application
    onUpperGradientToggle(false);
    onSoleGradientToggle(false);
    
    // Call the parent's colorway select handler
    onColorwaySelect(colorway);
  };
        ...
        return (
          <div className="space-y-4">
            <ColorwaySelector
              ...
              onColorwaySelect={handleColorwaySelect}
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-20 px-4 pb-4">
      ...
      <div ref={customizerRef} className="space-y-3">
            ...
            {isAIChatActive ? (
              ...
              >
                ...
                <div
                  ...
                  style={{
                    width: 'calc(50% - 2px)',
                    left: `calc(${activeTab === 'logos' ? '50' : '0'}% + 1px)`
                  }}
                />
        ...
        {/* Main Control Card */}
        <div className={` max-w-6xl mx-auto min-h-[80px] rounded-[20px] transition-all duration-300 ${isDarkMode ? 'bg-transparent ' : 'bg-transparent'}`}>
          {/* Main Control Bar */}
            ...
            <div className="flex items-center justify-between gap-2 sm:gap-4">
              ...
              <div className="flex-1 flex justify-center">
                ...
                {activeTab === 'upper' && (
                  <div className="text-center">
                    <p className={`text-sm transition-all duration-300 ${isDarkMode ? 'text-white/60' : 'text-muted-foreground'}`}>
                      Select a colorway below to customize your shoe
                    </p>
                  </div>
                )}
                {activeTab === 'logos' && (
                  <div className="text-center">
                    <p className={`text-sm transition-all duration-300 ${isDarkMode ? 'text-white/60' : 'text-muted-foreground'}`}>
                      Upload and customize your logos
                    </p>
                  </div>
                )}
                    <Popover>
